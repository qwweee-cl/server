...
echo $nowDate
if [ -z "$1" ]; then
    echo "ymkEvent No argument 1 supplied"
else
	nowDate=$1
fi
echo $nowDate

if [ $2 = "countly_raw0" ]; then
	liveconf=0
	rm $file -rf
...
#		remotedb="test_raw0"
		...
		echo -e "Clad YMK Event failed clad.cyberlink.com>/usr/local/countly/log/ymkEvent_batch.log" $(tail -20 /usr/local/countly/log/ymkEvent_batch.log)\
		| mail -s "Clad YMK Event Error argument" gary_huang@cyberlink.com,snow_chen@cyberlink.com,qwweee@gmail.com
		exit 0
	fi
fi

# save rawlog to mysql log
node saveMysql.js $batchdb
#node processData.js
