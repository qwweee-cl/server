...
if [ -e ${LOCKFILE} ] ; then
	...
	| mail -s "Main Countly Batch Already running" Gary_Huang@PerfectCorp.com,qwweee@gmail.com
	#sleep 1
	rm -f ${LOCKFILE}
	exit 1
else
...
#echo $cmd
#$cmd
cmd="ssh ubuntu@clad2 /usr/local/countly/api/hourlySlaveCountlyBatch.sh $batchdb >> /usr/local/countly/log/slave_batch.log &"
echo $cmd
$cmd

cmd="ssh ubuntu@clad3 /usr/local/countly/api/hourlySlaveCountlyBatch.sh $batchdb >> /usr/local/countly/log/slave_batch.log &"
echo $cmd
$cmd
...
| mail -s "Main [$curdate]Countly Batch Finished" Gary_Huang@PerfectCorp.com,qwweee@gmail.com
#sleep 1
rm -f ${LOCKFILE}
