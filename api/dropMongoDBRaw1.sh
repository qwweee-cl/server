...
function error_exp
{
	mail -s "[clad1] ${fullCurDate} MongoDB drop database exception (${string})" ${dropMongo} < ${one_time_log}
	exit 0
}
...
mail -s "[clad1] ${fullCurDate} Finish drop database" ${dropMongo} < ${one_time_log}
