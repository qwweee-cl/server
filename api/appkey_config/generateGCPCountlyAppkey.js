...
const tmpString2 = "}s:4:\"name\";s:27:\"";
...
const tmpString22 = "}s:4:\"name\";s:23:\"";
...
for (i = 0; i < 1000; i++) {
  ...
  const name = "SDK_GCP_AUTO_Android_" + padLeft(i, 6);
...
for (i = 0; i < 1000; i++) {
  const objectId = mongoObjectId();
  const name = "SDK_GCP_AUTO_iOS_" + padLeft(i, 6);
  const appKey = sha1Hash(objectId, true);
  const newData = {
    "_id": objectId,
    "name": name,
...
const dataFile1 = path.join('./', 'countly_gcp_auto_android_mongo.json');
...
const dataFile2 = path.join('./', 'countly_gcp_auto_ios_mongo.json');
...
const dataFile11 = path.join('./', 'countly_gcp_auto_android_json.json');
...
const dataFile12 = path.join('./', 'countly_gcp_auto_ios_json.json');
