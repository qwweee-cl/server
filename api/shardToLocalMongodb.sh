...
function error_exp
{
	...
	| mail -s "[Wrong][Shard] Session${indexNum} ${startDate}_${startRound} S3 to Local Mongodb Trap" ${AWSM}
	exit 1
}

function sendSummaryMail() {
	echo -e $(cat $one_day_log)\
	| mail -s "[Shard] Session${indexNum} ${startDate}_${startRound} S3 to Local Mongodb Summary" ${AWSM}
}

log_path="/usr/local/countly/log/shardSession"
working_dir="/usr/local/countly/api"
mail_target=${AWSM}
one_day_log="$log_path/log_mongotolocal_$(date +%Y%m%d).log"
...
## download file use aws cli from s3 to local shard2FileName
cmd="aws s3 cp ${s3DownloadPath}${shard2FileName} ${downloadPath}${shard2FileName}"
echo -e "${cmd}" 2>&1 >> ${one_day_log}
${cmd} 2>&1 >> ${one_day_log}

## untar shard2FileName to untarPath
cmd="tar xzf ${downloadPath}${shard2FileName} -C ${untarPath} ${untarFiles}"
echo -e "${cmd}" 2>&1 >> ${one_day_log}
${cmd} 2>&1 >> ${one_day_log}

## rm shard2FileName download file
cmd="rm -rf ${downloadPath}${shard2FileName}"
echo -e "${cmd}" 2>&1 >> ${one_day_log}
${cmd} 2>&1 >> ${one_day_log}

## mongorestore files to local mongodb
cmd="mongorestore -d ${dbName} ${untarPath}${startDate}_raw_${startRound}_2/${dbName}"
echo -e "${cmd}" 2>&1 >> ${one_day_log}
${cmd} 2>&1 >> ${one_day_log}

## rm untar files
cmd="rm -rf ${untarPath}${startDate}_raw_${startRound}_2"
echo -e "${cmd}" 2>&1 >> ${one_day_log}
${cmd} 2>&1 >> ${one_day_log}
