...
function error_exp
{
	echo -e "[ShardTest]Mongo to Mysql Error Please check log in claddb2.cyberlink.com>"\
	"${alllogpath}mongoToMysql.log" $(tail -20 ${alllogpath}mongoToMysql.log)\
	| mail -s "[ShardTest]Mongo to Mysql Error Trap" ${AWSM}
	exit 0
}
...
echo $end
echo "==============================================================="
echo -e "[ShardTest Fill Zero]Mongo to Mysql run from $start to $end\n" $(tail -20 ${alllogpath}mongoToMysql.log)\
| mail -s "[ShardTest Fill Zero]Claddb2 [${curdate}] Mongo to Mysql Finished" ${AWSM}
